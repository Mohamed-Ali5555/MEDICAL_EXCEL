...
public static function scopeSameCountry($query)
{

  $query->whereNull('city_id')
    ->orWhereIn('city_id', function ($sub) {
      $sub->from('cites')->select('id')
        ->where('iso', session('country_code'));
    })->orWhereIn('id', function ($sub) {
      $sub->from('listing_countries')->select('listing_id')
      ->where('country_id', session('country_code'));
    });
}
public static function scopeSeameCountry($query)
{

  $query->whereNull('country_id')
    ->orWhereIn('country_id', function ($sub) {
      $sub->from('countries')->select('id')
        ->where('iso', session('country_code'));
    })->orWhereIn('id', function ($sub) {
      $sub->from('listing_countries')->select('listing_id')
      ->where('country_id', session('country_code'));
