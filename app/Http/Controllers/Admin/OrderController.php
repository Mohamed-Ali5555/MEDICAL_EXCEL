...
/////////////////////
/////////////////////
public function collect_details($collect_id){
    // return $collect_id;
    $company_name =BusinessSetting::where('type', 'company_name')->first()->value;
    $company_web_logo =BusinessSetting::where('type', 'company_web_logo')->first()->value;
    $order = Order::with('details.product_all_status', 'shipping', 'seller.shop')->where(['collect_id' => $collect_id])->first();
    $collect_order_detail = Order::with('details.product_all_status', 'shipping', 'seller.shop')
...
$linked_orders = Order::where(['order_group_id' => $order['order_group_id']])
    ...
    return view('admin-views.order.collect_details', 
    compact('collect_order_detail',
    'order','linked_orders',
    'physical_product'
    
));
// }else{
//     return view('admin-views.pos.order.order-collect_details', compact('order', 'company_name', 'company_web_logo'));
// }
}
...
////////////////////
    ...
    // public function generate_invoice($collectId)
    // {
    ...
    //     $company_name = BusinessSetting::where('type', 'company_name')->value('value');
    //     $company_web_logo = BusinessSetting::where('type', 'company_web_logo')->value('value');
    
    //     $order = Order::with(['seller', 'shipping', 'details'])
    //         ->where('collect_id', $collectId)
    //         ->firstOrFail();
    
    //     $seller = Seller::findOrFail($order->details->first()->seller_id);
    
    //     $data["email"] = optional($order->customer)->email ?: \App\CPU\translate('email_not_found');
    //     $data["client_name"] = optional($order->customer)->f_name . ' ' . optional($order->customer)->l_name ?: \App\CPU\translate('customer_not_found');
    //     $data["order"] = $order;
    
    //     $mpdf_view = view('admin-views.order.invoice', compact('order', 'seller', 'company_phone', 'company_name', 'company_email', 'company_web_logo'));
    //     Helpers::gen_mpdf($mpdf_view, 'order_invoice_', $order->id);
    // }
    ...
    public function collect_order(Request $request, $status){
//////

// $newCollectId = "COLLECT_" . uniqid(); 
        //////
        $search = $request['search'];
        $filter = $request['filter'];
        $from = $request['from'];
        ...
        $orders = Order::with(['customer', 'seller.shop'])
            ...
            ->select('id','collect_id', 'customer_id', 'seller_id','seller_is', 'payment_status', 'order_status',
           
            DB::raw('SUM(order_amount) as total_amount'))
            ->groupBy('customer_id', 'collect_id')
        ...
        return view(
            'admin-views.order.collect_order',    compact(
                'orders',
                'search',
                'from', 'to', 'status',
                'filter',
